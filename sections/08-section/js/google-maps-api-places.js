/**
 * Google Maps Samples Autocomplete
 * 
 * English:
 * Sample of usage of autocomplete.
 * 
 * Portuguese: 
 * Amostra de uso de preenchimento automático.
 * 
 */
...
function initMap () {
  ...
  */
  ...
  var timeAutocomplete = new google.maps.places.Autocomplete (document.getElementById('search-within-time-input'));

  var zoomAutocomplete = new google.maps.places.Autocomplete (document.getElementById('search'));
...
/**
 * Function to hide the specific list of markers
 * @param {markersArray} markers 
 */
...
function displayDirections (origin) {
  ...
  let destinationAddress = document.getElementById('search-within-time-input').value;
  ...
  directionsService.route (
    ...
    function (response, status) {
      if (status === google.maps.DirectionsStatus.OK) {
        ...
        window.alert ("Directions request failed / Falha na Requisição de Direções " + status);
      }
    }
  );
};
...
function searchBoxPlaces (searchBox) {
  ...
  if (places.length == 0) {
    window.alert ('Not find that search place!');
  };
};
    ...
    marker.addListener ('click',
      function () {
        if (placeInfoWindow.marker == this){
          console.log("Infowindow is on this marker");
        } else {
          getPlacesDetail (this, placeInfoWindow);
        }
      }
    );
    ...
    function (place, status) {
      if (status === google.maps.places.PlacesServiceStatus.OK) {
        ...
        if (place.opening_hours) {
          innerHTML += '<br><br><strong>Opening Hours:</strong><br>' +
        ...
        if (place.photos) {
          innerHTML += '<br><img src="' +
            place.photos[0].getUrl ({maxHeight: 100}) + '">';
        }
